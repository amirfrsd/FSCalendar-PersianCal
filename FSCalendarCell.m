...
- (void)configureCell
{
    ...
    NSDictionary *numbersDictionary = @{@"0" : @"۰",
                                        @"1" : @"۱",
                                        @"2" : @"۲",
                                        @"3" : @"۳",
                                        @"4" : @"۴",
    ...
    for (NSString *key in numbersDictionary) {
        _titleLabel.text = [_titleLabel.text stringByReplacingOccurrencesOfString:key withString:numbersDictionary[key]];
    }
...
- (BOOL)isWeekend
{
    return _date && ([_calendar weekdayOfDate:_date] == 6);
}
